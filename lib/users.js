...
export const TAB_DEFS = [
  ...
  { key: 'wd',       label: '주중예배' },
  { key: 'district', label: '장년교구' },
  { key: 'newfam',   label: '새가족' },
  { key: 'special',  label: '특별예배' },
  { key: 'yearcomp', label: '연도별 비교' },
];
export const ALL_TABS = TAB_DEFS.map(t => t.key);

/* 아이디 규칙 — 교역자 이름을 그대로 쓰므로 한글을 허용한다.
   교적 알파벳(김정은AD 처럼)이 붙는 경우가 있어 대문자도 그대로 둔다.
   찾을 때만 대소문자를 무시하므로 소문자로 쳐도 들어온다. */
export const ID_RE = /^[가-힣ㄱ-ㅎA-Za-z0-9._-]{2,20}$/;
export const normId = (v) => String(v ?? '').trim();
const foldId = (v) => normId(v).toLowerCase();
...
export function findUser(store, id) {
  const k = foldId(id);
  return listUsers(store).find(u => foldId(u.id) === k) || null;
}
...
export function grantsOf(user) {
  ...
  return {
    admin,
    // 주간현황을 자동으로 붙이지 않는다. 부서 담당자는 자기 부서만 봐야 하는데
    // 여기서 끼워 넣으면 교회 전체 숫자가 열린다.
    tabs,
    // null = 제한 없음, 배열 = 그 항목만
